 ...
 */
function Chart({data}: { data: PriceHistory }) {
    ...
    const startDate = Math.max(oneYearAgo.getTime(), Math.min(...data.map(d => d.date.getTime())));
    const domain = [new Date(startDate), now];
            ...
            <ResponsiveContainer>
                <LineChart data={data}>
                    <XAxis dataKey="date"
                           ...
                           ticks={getTicks(new Date(startDate), now, 12)}/>
                    <YAxis/>
